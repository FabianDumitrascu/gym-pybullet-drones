...
def add_custom_obstacles(client):
    ...
    # Load block 1
    beam_id = p.loadURDF("../assets/beam.urdf", [0.0, 1.0, 0.75], p.getQuaternionFromEuler([0, 0, 0]), physicsClientId=client)
    ...
    # Load block 3
    cube_id = p.loadURDF("../assets/cube.urdf", [1.0, 2.5, 0.5], p.getQuaternionFromEuler([0, 0, 0]), physicsClientId=client)

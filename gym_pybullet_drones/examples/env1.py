...
def add_custom_obstacles(client):
    ...
    # Load block 1
    beam_id = p.loadURDF("../assets/beam.urdf", [0.5, 1.0, 0.75], p.getQuaternionFromEuler([0, 0, 0]), physicsClientId=client)
    obstacle_ids.append(beam_id)

    # Load block 2
    beam2_id = p.loadURDF("../assets/beam_horizontal.urdf", [2.0, 2.5, 0.25], p.getQuaternionFromEuler([0, 0, 0]), physicsClientId=client)
    obstacle_ids.append(beam2_id)

    # Load block 3
    cube_id = p.loadURDF("../assets/cube.urdf", [2.0, 0.5, 0.5], p.getQuaternionFromEuler([0, 0, 0]), physicsClientId=client)
